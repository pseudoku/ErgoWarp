...
ColumnOrigin =  //Origin of the arc
...
OriginCnRm = //switch reference origin
[
  [ //C0
...
// derived transform from polar to cartesian with chain rule
...
[  
    //X
    PathStruct(theta)[column][side][PATH]*cos(theta)+
    rotateVecZ(ShapeOrigin, atan(derPathY(theta,column,side)/derPathX(theta,column,side))+angleOffset)[0], 
    //Y
    PathStruct(theta)[column][side][PATH]*sin(theta)+
    rotateVecZ(ShapeOrigin, atan(derPathY(theta, column,side)/derPathX(theta,column,side))+angleOffset)[1], 
    //Z
...
module PlaceOnRoll(rollAngle =0 , offsets = -1,) 
{
...
} // set rotation origin on the plate top edge
...
module PlaceOnThumb(Rn = R0, stick = false)  
{
